...
def auto_input_generator():
    ...
    if run_dose == 'Y':
        ...
        input_dict = InpGenFunc.get_weathering_corr()
        print('\n')
            ...
            if have_dilution_factor == 'N':
                ...
                # no met data but long-term release; use conservative approach
                    ...
                    if like_to_scale_with_mean_speed == 'Y':
                        ...
                        print("dilution factor won't be scaled with meteorological data as the data is not provided by the "
    ...
    with open('%s.yaml' % input_file_name, 'w') as infile:
...
if __name__ == "__main__":
    input_file_name, file_exists, input_dict = InpGenFunc.get_input_file_name()
    ...
    subprocess.call("python main.py --config_file %s.yaml --output_file_name %s --logdir %s" % (
    input_file_name, input_dict['output_file_name'], input_dict['logdir_name']), shell=True)
