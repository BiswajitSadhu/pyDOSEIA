...
def test_main():
    ...
    logging.basicConfig(filename='%s/TEST1.log' % args.logdir, level=logging.INFO, filemode="w+")
    ...
    ########## TEST 2 ##################################################################################################
    # subprocess.call("python main.py --config_file test_inhal_gs.yaml --output_file_name test_out --logdir test", shell=True)
    args = parse_arguments()
    logging.basicConfig(filename='%s/TEST2.log' % args.logdir, level=logging.INFO, filemode="w+")
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
